...
# Input paths
BANDPASSED_CSV_ROOT = "/home/owen/etna_signals_phd/bandpassed_exports_001_01_csv_only_utc"
TEMPLATES_CSV_ROOT = "/home/owen/etna_signals_phd/tilt_templates_csv"
EARTHQUAKE_CSV = "/home/owen/etna_signals_phd/earthquakes_merged_utc.csv"

# Output paths
OUTPUT_ROOT = "/home/owen/etna_signals_phd/SWCC_utc_fixed"
...
VOLCANIC_EVENTS_CSV = "/home/owen/etna_signals_phd/etna_volcanic_events_cleaned.csv"
...
def remove_p_wave_contaminated_periods(time_dt, signal, earthquake_df):
    ...
    for _, quake in earthquake_df.iterrows():
        ...
        # Asymmetric buffer: pre (before ETA) and post (after ETA, extended for large quakes)
...
def main():
    ...
    for DATASET, STATIONS in DATASETS_STATIONS.items():
        ...
        for STATION in STATIONS:
            ...
            for sim in sims:
                # Skip ECPN sim1 and sim2 (already processed)
                if STATION == "ECPN" and sim in ["sim1", "sim2"]:
                    print(f"\n{'='*80}")
                    print(f"⏭️  Skipping {STATION} {sim.upper()} (already processed)")
                    print(f"{'='*80}\n")
                    continue
