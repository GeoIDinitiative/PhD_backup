...
def detect(gt, score, veto, fdet, fsig, M=PEAK_SEP):
    """find_peaks is wired to the DETECTION floor (95th-pct null); peaks above the
    SIGNIFICANCE floor (99th-pct null) are flagged significant. Returns (detections,
...
def main():
    ...
    for ds in STATIONS:
        cs = c[c.dataset == ds]
        L.append(f"   {ds:11s}: MAX  detect={cs.max_detect.sum()} signif={cs.max_signif.sum()} | "
                 f"STACK detect={cs.stack_detect.sum()} signif={cs.stack_signif.sum()}")
    L.append("")
    ...
    for col, method in enumerate(["max", "stack"]):
        ...
        axes[col].set_title(f"continuous {method.upper()}: synchrony vs chance")
        axes[col].set_xlabel("coincident events"); axes[col].legend(fontsize=8); axes[col].grid(alpha=0.3)
        L.append("")
